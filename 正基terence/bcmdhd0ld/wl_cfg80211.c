...
static u32 wl_get_suite_auth_key_mgmt_type(uint8 type,int wpa_type)
{
	u32 ret = 0;
	switch (type) {
		case RSN_AKM_NONE:
	...
	bcm_tlv_t *wpa2ie, s32 bssidx)
{
	...
	for (i = 0; i < count && len >= WPA_SUITE_LEN;
		...
		if (!bcmp(mgmt->list[i].oui, WPA_OUI, WPA_OUI_LEN)) {
			if (IS_WPA_AKM(mgmt->list[i].type)) {
				...
				wpa_auth1 |= wl_get_suite_auth_key_mgmt_type(mgmt->list[i].type,1);
			}
		}
	...
	suite_count = ltoh16_ua(&mgmt->count);
	ptmp = (u8 *)&mgmt->list[0];
	wpa_auth2 = wl_get_suite_auth_key_mgmt_type(ptmp[DOT11_OUI_LEN],2);
	WL_ERR(("wpa ie wpa_suite_auth_key_mgmt count=%d, key_mgmt = 0x%X\n", count, wpa_auth2));
